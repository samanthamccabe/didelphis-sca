...
import org.didelphis.common.language.machines.Machine;
import org.didelphis.common.language.machines.StateMachine;
...
public class Condition {
	...
	public boolean isMatch(Sequence word, int startIndex, int endIndex) {
		...
		if (endIndex <= word.size() && startIndex <= endIndex) {
			Sequence head = word.subsequence(0, startIndex);
			Sequence tail = word.subsequence(endIndex);

			Sequence reverse = new BasicSequence(head);
