...
\begin{description}
\item[Descritpion] contrasts sounds in which the vocal cords are in a modal position (voiced) from one in which they are open (voiceless)
\item[Type] \emph{binary}
\item[Examples]\
  \begin{itemize}
    \item \texttt{[+voice]}\\
...
\begin{description}
\item[Descritpion] %an \emph{ad-hoc} feature designed to contrast voiceless from breathy, and voiced from creaky phonation under the rationalizatin that breathy and creaky voice both have more restricted glottal configurations than voiceless and voiced respectively.

% TODO: this won't work --- breathy and creaky both pattern with voices sounds, so representing breathy as [+tense, -voice] is wrong
% TODO: we cold make VOT breathy again or something;spread glottis.
\item[Type] \emph{binary}
\item[Examples]\
  \begin{itemize}
    \item \texttt{[+tense]}\\
